    ...
    @cutotune(
        configs=(
            get_cartesian_product_cutotune_configs(
                kernel_backend=[KernelBackend.cuda], BLOCK_SIZE=BLOCK_SIZES_POWERS_OF_2
            )
    ...
    def forward(
        ...
        if kernel_backend == KernelBackend.cuda:
            if torch.compiler.is_compiling():
                add_tensor_forward_cuda_kernel_compileable(x=x, y=y, output=output, BLOCK_SIZE=BLOCK_SIZE)
            else:
...
def add_tensor_khd(
    ...
    return _AddTensor_KHD.apply(x, y, kernel_backend, BLOCK_SIZE)
