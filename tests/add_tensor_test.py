...
class AddTensorTest(TestCommons):
    @parameterized.expand(
        TestCommons.make_args_matrix(
            ...
            [
                partial(add_tensor_cuda, v_size=1),
                partial(add_tensor_cuda, v_size=2),
                partial(add_tensor_cuda, v_size=4),
                torch.compile(partial(add_tensor_cuda, v_size=1)),
                torch.compile(partial(add_tensor_cuda, v_size=2)),
                torch.compile(partial(add_tensor_cuda, v_size=4)),
                add_tensor_triton,
        ...
        + TestCommons.make_args_matrix(
            ...
            [
                partial(add_tensor_cuda, v_size=8),
                torch.compile(partial(add_tensor_cuda, v_size=8)),
            ],
        )
    )
