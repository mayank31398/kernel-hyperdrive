...
__global__ void _add_scalar_forward_cuda_kernel(const scalar_t *x,
    ...
    if constexpr (vector_instruction_width == 1) {
        if (thread_id < num_elements) {
            output[thread_id] = dtype::downcast(dtype::upcast(x[thread_id]) + y);
        }
    } else {
...
void add_scalar_forward_cuda(const torch::Tensor x,
    ...
    AT_DISPATCH_CUSTOM_FLOAT_TYPES(
        x.scalar_type(), "add_scalar_forward_cuda_kernel", ([&] {
            ...
            case 1:
                _add_scalar_forward_cuda_kernel<scalar_t, scalar_t, 1>
            ...
            case 2:
                ...
                _add_scalar_forward_cuda_kernel<scalar_t, vector_t, 2>
                    <<<NUM_BLOCKS, BLOCK_SIZE>>>(x.data_ptr<scalar_t>(), y, output.data_ptr<scalar_t>(), num_elements);
                break;
            case 4:
                if constexpr (std::is_same_v<scalar_t, fp32>) {
                    _add_scalar_forward_cuda_kernel<scalar_t, fp32_4, 4><<<NUM_BLOCKS, BLOCK_SIZE>>>(
                        x.data_ptr<scalar_t>(), y, output.data_ptr<scalar_t>(), num_elements);
                } else {
                    _add_scalar_forward_cuda_kernel<scalar_t, fp32_2, 4><<<NUM_BLOCKS, BLOCK_SIZE>>>(
                        x.data_ptr<scalar_t>(), y, output.data_ptr<scalar_t>(), num_elements);
                }
                break;
            case 8:
                if constexpr (std::is_same_v<scalar_t, fp32>) {
                    throw std::runtime_error("fp32 doesn't support vector_instruction_width of 8");
                } else {
                    _add_scalar_forward_cuda_kernel<scalar_t, fp32_4, 8><<<NUM_BLOCKS, BLOCK_SIZE>>>(
                        x.data_ptr<scalar_t>(), y, output.data_ptr<scalar_t>(), num_elements);
                }
                break;
            default:
