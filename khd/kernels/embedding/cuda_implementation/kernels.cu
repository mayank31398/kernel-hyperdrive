...
__global__ void _embedding_forward_cuda_kernel(const scalar_t *x,
                                               const scalar_t *y,
                                               scalar_t *output,
...
void embedding_forward_cuda(const torch::Tensor x,
    ...
    AT_DISPATCH_CUSTOM_FLOAT_TYPES(
        x.scalar_type(), "embedding_forward_cuda_kernel", ([&] {
            const int vector_instruction_width = sizeof(fp32_4) / sizeof(scalar_t);

            const int num_elements_per_block = BLOCK_SIZE * vector_instruction_width;
            const int NUM_BLOCKS = (num_elements + num_elements_per_block - 1) / num_elements_per_block;

            _embedding_forward_cuda_kernel<scalar_t><<<NUM_BLOCKS, BLOCK_SIZE>>>(
